...
// This was copied from the mileageLogHandler, hence the similarities in layout.
...
function runUpdates(): void {
    ...
    // double-cachelock
    if (locker.isPrimaryLocked && liveConfig.enable_cacheLock) {
    ...
    for (let rawResponse of responseData.data) {
        ...
        // Probably a bigger problem on the mileageLog system.
        if (checkTime_(startTime, softCutoffInMinutes)) {
            let response: carCheckEntry = convertKiEntryToCarCheckEntry_(rawResponse);
